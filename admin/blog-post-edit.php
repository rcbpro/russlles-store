...
?>
	<div id="masthead"><br /><br /><br />
		<div class="content_pad">
			<h1 class="no_breadcrumbs">Blog Post Edit</h1>
			<!-- #search -->
		</div> <!-- .content_pad -->
	</div> <!-- #masthead -->	
	
	<div id="content" class="xgrid">
		<?php if ((isset($_SESSION['blog_success_update'])) && ($_SESSION['blog_success_update'])):?>
        <div class="success">Post updated successfully !</div>
        <?php unset($_SESSION['blog_success_update'])?>
        <?php endif;?><br />
        <?php if ((isset($_SESSION['blog_fail_update'])) && ($_SESSION['blog_fail_update'])):?>
        <div class="error">Post updated failed !</div>
        <?php unset($_SESSION['blog_fail_update'])?>
        <?php endif;?><br />
        <?php if ((isset($_SESSION['image_restricted'])) && ($_SESSION['image_restricted'])):?>
        <div class="error">Not a valid type to upload !</div>
        <?php unset($_SESSION['image_restricted'])?>
        <?php endif;?><br />
		<div class="x12"> 
		<form id="blog_post" name="blog_post" method="post" class="form label-inline uniform" action="blog-post-edit-save.php" enctype="multipart/form-data">
            <input type="hidden" name="blog_post_id" id="blog_post_id" value="<?php echo (!empty($blog_post_id)) ? $blog_post_id : ""?>" />
            <div class="field">
                <label for="name">Title</label> 
                <input id="title" name="post_title" size="100" type="text" class="medium" value="<?php echo ($post_title != "") ? $post_title : ""?>" />
            </div> 
            <div class="field">
            ...
            <span class="label">Is Active?</span>
			   ...
			   ?>
                  ...
                  <img id="main_image" name="main_image" src="../images/blog-post/<?=(($cover_image != "") ? $cover_image : "")?>" border="0">
                  </div>
               <?php endif; ?>
            <div class="field">
            ...
            <div class="buttonrow">
                ...
                <button class="btn btn-grey" type="button" onClick="javascript: get_confirmation('Are you sure you wish to delete this blog post? ','blog-post-delete.php?id=<?=$blog_post_id?>')">Delete </button>
                <? endif; ?>
            </div>
        </form>
		</div> <!-- .x12 -->
