...
if ($blog_post_id == 0){
	$sql = "INSERT INTO posts(post_title, post_content, active, date_created, post_type) VALUES('".$post_title."', '".$post_content."', ".$active.", '".date("Y-m-d")."', 'N')";
			 ...
			 //if(($image_size[0]==$width && $image_size[1]==$height)||$image_type==3){ 
			 	rename($upload_directory.'temp-'.$image_save_name,$upload_directory.$image_save_name);
			  	$sql = "UPDATE posts SET
	       						...
	       						WHERE post_id = ".$blog_post_id." AND post_type = 'N'";
				$DBConn->execute_query($sql);
				$_SESSION['news_success_update'] = true;
			 //}else{
//				 unlink($upload_directory.'temp-'.$image_save_name);
//				 $message_id=17;//Image size incorrect
//			 }
		 }else{
			$_SESSION['news_fail_update'] = true;		 
		 }
	 }else{
		$_SESSION['news_image_restricted'] = true;		 
	 }
}else{
	$sql = "UPDATE posts SET
						post_title = '".mysql_preperation($post_title)."',
...
header("Location: news-edit.php?id=".$blog_post_id);
exit();
